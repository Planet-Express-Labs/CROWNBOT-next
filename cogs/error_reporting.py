...
class Reporting(commands.Cog):
    ...
    async def report_error(self, ctx, *, error_message):
        ...
        embed = disnake.Embed(title="User reported error", description=error_message)
    ...
    async def on_ready(self):
        embed = disnake.Embed(title="Bot on", description="Bot is now online. ")
    ...
    async def on_command_error(self, ctx: commands.Context, error: commands.CommandError):
        ...
        if not isinstance(error, commands.CommandNotFound):
            embed = disnake.Embed(title="Command error", description=str(error))
            embed.set_author(name=ctx.author.name, icon_url=ctx.author.avatar_url)
    ...
    async def on_slash_command_error(self, inter: disnake.AppCmdInter, error: commands.CommandError):
        ...
        embed = disnake.Embed(title="Slash command error", description=str(error))
        embed.set_author(name=inter.author.name, icon_url=inter.author.avatar_url)
